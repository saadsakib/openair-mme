...
int s6a_aia_cb(struct msg **msg, struct avp *paramavp, struct session *sess,
  ...
  if (avp) {
    CHECK_FCT(fd_msg_avp_hdr(avp, &hdr));
    snprintf(s6a_auth_info_ans_p->imsi, (int)hdr->avp_value->os.len + 1, "%*s",
  ...
  if (avp) {
    ...
    MSC_LOG_TX_MESSAGE(
        MSC_S6A_MME, MSC_NAS_MME, NULL, 0, "0 S6A_AUTH_INFO_ANS imsi %s %s",
    ...
    if (avp) {
      ...
      MSC_LOG_TX_MESSAGE(MSC_S6A_MME, MSC_NAS_MME, NULL, 0,
       ...
       */
      MSC_LOG_TX_MESSAGE_FAILED(MSC_S6A_MME, MSC_NAS_MME, NULL, 0,
...
int s6a_generate_authentication_info_req(s6a_auth_info_req_t *air_p) {
   ...
   */
  {
    ...
    if (air_p->re_synchronization) {
      OAILOG_DEBUG(LOG_S6A, "Added Re-Synchronistaion for UE \n");
